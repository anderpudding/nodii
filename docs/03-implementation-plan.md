...
- [x] 설정 화면: 주 시작 요일, 테마, 로그아웃, **계정 삭제**, 개인정보처리방침·이용약관·지원 링크, 앱 버전
- [x] 단축키: `⌘N`, `⌘T`, `←/→`, `⌘,` (메뉴와 연결)
- [x] 드래그 정렬(목표, 할 일), HEX 색상 입력, 목표 완전 삭제 확인 창, 할 일 삭제 실행 취소 검증(3단계 구현)
- [x] Could 항목(메뉴 막대 빠른 추가, 캘린더로 드래그, 다른 목표로 이동, 복제)은 출시 후로

**구현·검증 기록 (2026-09-22)**
- GOAL-03/04/06·TODO-06: HEX 입력 검증·대비 미리보기, 활성 목표 및 같은 목표 안 할 일 정렬, 한국어 키보드 드래그 안내, 손잡이/8px 활성화 거리. 루틴은 드롭 이웃으로 포함하되 직접 드래그는 불가. 이동한 행 하나의 `sortKey`만 갱신하며 실패하면 겹치는 월 캐시도 롤백.
- GOAL-06: 관리 시트 전체의 개수를 관계 집계 한 번으로 조회(1,000개 목표를 넘으면 페이지 추가), 삭제 확인은 `count: exact, head: true`로 최신 개수 재확인. 목표·할 일·루틴을 한 트랜잭션에서 소프트 삭제하고 마지막 활성 목표·타인·anon은 DB에서도 거부. 클라이언트는 관련 캐시에서 즉시 제거하고 실패한 목표의 행만 복원.
- TODO-04: 기존 5초 토스트를 재사용하며 연속 삭제의 독립적인 실행 취소, 원래 날짜/키 복원 검증. AUTH-06: 계정 삭제 후 서버 signOut 오류에도 로컬 캐시·세션 정리 검증. 로그아웃의 갱신 정지는 유지하고 재로그인/창 표시에서 재개하도록 수명 관리 보완.
- 검증: `pnpm lint`, `pnpm format:check`, `pnpm typecheck`, `pnpm test` 통과(core 156 + api 74 + desktop 94 = 324검사). `pnpm build:web` 통과. JS 본문 약 800KB로 기존 500KB 청크 경고는 남음. A 묶음에서 `pnpm db:reset`, `pnpm db:test`(8파일 138검사), `pnpm db:types` 통과했고 이후 DB 변경 없음.
- 실제 연동: 로컬 OTP 일회용 계정으로 목표별 관계 집계 HTTP 200 및 계정 삭제 HTTP 204 확인 후 정리. `pnpm dev` 실행 성공. 자동화 도구의 기존 설치본 선택을 피하려고 같은 debug 실행 파일을 `/private/tmp` 임시 앱으로 감싸 네이티브 로그인 화면·한국어 메뉴 및 입력창 ⌘A/⌘C/⌘X/⌘V 유지 확인(저장소 번들 ID/Info.plist 변경 없음).
- 의존성: `@dnd-kit/core` 6.3.1, `@dnd-kit/sortable` 10.0.0 — 포인터/키보드 정렬. 간접 의존성 accessibility/utilities 추가. 새 Tauri capability·플러그인·Rust 변경 없음. 메뉴는 기존 `core:default`, 외부 링크는 기존 opener 권한 사용.
- 문서 차이/결정: 설정은 7단계 지시서의 `SettingsSheet` 채택(화면 스펙의 별도 창과 차이). 계정 삭제는 디자인의 개수·이해 체크와 지시서의 이메일 재입력을 모두 요구. 정렬 이웃 키가 동시 편집으로 같아진 경우는 한 행 UPDATE 원칙을 유지해 오류 안내 후 재시도하며 자동 전체 재정렬은 하지 않음. 디자인에 드래그 상태 스펙이 없어 기존 표면·모션·간격 토큰을 재사용.
- 사람이 확인할 것: 테스트 계정 삭제 후 Studio의 사용자/하위 데이터 제거, 로그인한 Tauri에서 ⌘N/⌘T/⌘,/⌘W/⌘Q·최소화, 실제 포인터/키보드 드래그 후 재실행·다른 창에서 순서 유지, 테마의 plugin-store 재실행 유지. 배포용 개인정보처리방침·이용약관·지원 HTTPS URL을 환경에 설정(미설정 시 준비 중). Could 항목은 구현하지 않았으며 출시 후로 유지.

### 7d · 실사용 피드백 반영

- [x] F1·F2: 창 높이 고정, 목표별 목록과 관리 시트·루틴 편집 본문만 스크롤. 스크롤바 숨김, 키보드 포커스 여백, 목표 추가·루틴 저장 버튼 고정.
- [x] TODO-10~12: 가져오기 배너를 하루 메뉴로 통합. 선택 날짜의 미완료 할 일만 일괄 이동·삭제, 개수 확인, 오프라인/쓰기 중 비활성 이유, 5초 실행 취소.
- [x] API: 200개 단위 일괄 소프트 삭제·복원, 기존 `move_todos`와 월/overdue 캐시 재사용, 낙관적 갱신·실패 롤백. 대상 날짜는 `profile.timezone`의 `buildDay`로 전개해 마지막 루틴 뒤에 정렬.
- [x] CAL-07·NFR-05: 최소 400×600, CSS 760px 미만 주간 스트립·상단 관리 메뉴. 공통 `CalendarGrid` 셀·키보드, 달을 걸치는 주는 두 월의 기존 캐시를 구독.
- [x] CAL-02·F5: 라이트/다크 `stamp-empty`, 다른 달 빈 상자 흐림, 빈 날짜의 스크린 리더 레이블 유지.
- [x] ROUT-10·F6: 하루 목록의 루틴 추가 제거, 루틴 관리의 새 루틴 및 기존 행 메뉴 유지.
- [x] `pnpm lint`, `pnpm format:check`, `pnpm typecheck`, `pnpm test` (core 156 + api 78 + desktop 104 = 338), `pnpm build:web` 통과.

7d 검증·제약:

- 브라우저 모의 데이터(할 일 40개·루틴 20개)로 1120×740과 400×600에서 문서 크기가 정확히 창 크기와 같고 가로 넘침이 없음을 확인. 목록만 세로 스크롤, 숨겨진 스크롤바, 키보드 정렬 중 목록 스크롤과 문서 스크롤 0, 759/760px 캘린더 전환, 좁은 창 관리 메뉴·관리 시트·루틴 편집의 고정 헤더/버튼, 삭제 확인의 취소 포커스, 다크 빈 도장 색 확인. 임시 검증 파일은 제거.
- 테스트: 일괄 삭제·복원 401개 분할과 오류 전파, 겹치는 월/overdue의 낙관적 갱신·롤백, 이동 전후 월 복원, 완료·루틴 제외, 세 동작 실행 취소, 대상 날짜의 루틴 뒤 정렬, 오프라인/쓰기 중 비활성, 주 시작 요일·월 경계·주 이동·키보드·선택 날짜 추적. CSS 전환은 jsdom의 `matchMedia`로 상태를 재구현하지 않고 실제 브라우저에서 검증하며, jsdom은 기본 넓은 창 가시성만 보조 CSS로 설정.
- 문서 차이: `tokens.md`의 최소 880px은 최신 지시서·요구사항의 400px을 우선. 날짜 선택은 지시서대로 기존 팝오버를 공유하며 `components.md`의 왼쪽 캘린더 선택 모드는 적용하지 않음. 일괄 삭제는 이번 지시서대로 되돌릴 수 있어도 확인 창 제공. 설계서 §7의 예전 배너 ASCII 그림보다 최신 표와 화면 스펙을 따름.
- 200개 초과 삭제/복원은 여러 HTTP 요청이므로 중간 실패 시 일부 서버 변경이 남을 수 있음. 캐시 롤백 후 서버 상태를 무효화·재조회하고 오류/재시도를 제공한다. DB 스키마·RPC 변경 없이 요청 전체의 원자성까지 보장하지는 않음.
- 새 의존성·Tauri 플러그인·capability·DB/core 변경 없음. DB 검증과 core 커버리지는 변경 범위가 없어 별도 실행하지 않음. 웹 빌드는 기존 500KB 경고 유지(JS 본문 807.45KB).
- 사람이 확인할 것: 실제 Tauri 창을 400×600 ↔ 1120×740으로 바꿀 때 자연스러운 전환, 트랙패드·포인터 드래그 자동 스크롤, 실데이터에서 하루 메뉴 세 동작과 각 실행 취소, 캘린더 도장/날짜 구분.
...
| 2026-09-19 | 7A | SET-01/02/07·AUTH-06 설정 시트, 기기 테마, 주 시작 낙관적 변경/월 캐시 무효화, 삭제 개수·이메일·이해 확인 및 계정 삭제 후 로컬 정리. GOAL-06 delete_goal RPC와 공용 개수 조회 기반 추가. 린트·포맷·타입·314검사 및 DB reset/test(138검사)/types 통과. 설정은 지시서의 SettingsSheet를 채택(화면 스펙의 별도 창과 차이), 삭제 확인은 두 문서 조건을 함께 적용. |

| 2026-09-19 | 7B | SET-03 공용 명령으로 브라우저 단축키·JS macOS 메뉴 연결. 날짜별 마지막 추가 목표/첫 활성 목표 입력 포커스, 편집·창 닫기·종료·최소화 predefined 메뉴 유지. 필수 검사 4종 및 316검사 통과. core:default의 기존 메뉴 권한 사용(새 capability·Rust 변경 없음). 실제 로컬 OTP로 공용 목표 집계 200·계정 삭제 204 확인. |
