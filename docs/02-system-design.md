...
> 색, 컴포넌트 상태, 실제 문구는 `DESIGN.md`와 `docs/design/`에 있습니다. 아래는 화면 구성만 다룹니다.
...
| 메인 (위 그림) | 왼쪽: 월간 캘린더와 관리 메뉴 / 오른쪽: 선택 날짜의 목표별 목록. 창 최소 크기 **400×600** |
| 메인 · 스크롤 (v1.4) | 창 높이에 맞춰 사이드바·상단 툴바·날짜 헤더는 고정하고 **목표별 목록 영역만 세로로 스크롤**한다. 시트·모달도 헤더는 고정하고 본문만 스크롤한다. 앱 안의 스크롤 영역은 스크롤바를 숨기되 휠·트랙패드·키보드 스크롤과 포커스 이동 시 자동 스크롤은 그대로 동작한다 |
| 메인 · 하루 메뉴 (v1.4) | 날짜 헤더 오른쪽 `⋯`. 항목: 지난 미완료 할 일 N개 가져오기(오늘일 때만, TODO-10) · 미완료 할 일 다른 날로 옮기기(날짜 선택 → `move_todos`, TODO-11) · 미완료 할 일 전체 삭제(확인 창 + 실행 취소, TODO-12). 오프라인이거나 쓰기 중이면 비활성 |
| 메인 · 좁은 창 (v1.4) | 폭이 좁으면 사이드바를 접고 월간 캘린더 대신 **주간 스트립**(선택 날짜가 있는 한 주 7칸, 이전·다음 주 이동, [오늘])을 날짜 헤더 위에 둔다. 목표 관리·루틴 관리 진입점은 상단 메뉴로 옮긴다. 조회는 넓을 때와 같은 월 캐시(`['todos', monthKey]`)를 쓴다 (CAL-07) |
...
| 루틴 추가 진입점 (v1.4) | 하루 목록에는 루틴 추가 버튼을 두지 않는다. 루틴 생성·수정·삭제는 루틴 관리 시트에서만 한다 (ROUT-10) |
